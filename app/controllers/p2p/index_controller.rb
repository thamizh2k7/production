...
class P2p::IndexController < ApplicationController
  ...
  def get_search_suggestions(query)
    ...
    item_result.each do |res|
      response.push({:label=> "#{res.title}" ,:value => URI.encode("/p2p/#{res.product.category.name}/#{res.product.name}/#{res.title}/#{res.id}") }) unless res.nil?
    end
    return response
  end
  ...
  def browse_filter
    ...
    items.each do |itm|
      ...
      temp_url = URI.encode("/p2p/#{itm.product.category.name}/#{itm.product.name}/#{itm.title}/#{itm.id}")
